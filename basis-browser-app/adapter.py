...
def _parse_report_votes(text):
    """Parse '6DP 2NR 1AM' into a dict like {DP: 6, NR: 2, AM: 1}.
    Also handles DNP. Returns (votes_dict, unanimous boolean)."""
    votes = {}
    for m in re.finditer(r'(\d+)(DP|DNP|NR|AM)', text):
        votes[m.group(2)] = int(m.group(1))
    dp = votes.get("DP", 0)
    dnp = votes.get("DNP", 0)
    nr = votes.get("NR", 0)
    am = votes.get("AM", 0)
    unanimous = (dnp == 0 and nr == 0 and am == 0 and dp > 0)
    return votes, unanimous


def committee_activity(session="34"):
    """Build committee activity data: reports, CS adopted, report unanimity."""
    import cache

    cached = cache.get("committee_activity")
    if cached is not None:
        return cached

    all_bills = _scan_all_actions(session)

    moved_out = []      # bills reported out of committee
    cs_adopted = []     # committee substitutes adopted
    report_unanimity = {"unanimous": 0, "divided": 0, "reports": []}

    for bn, origin, title, status, actions in all_bills:
        for code, achamber, jdate, text in actions:
            # Committee reports (moved out)
            if code == "002":
                cmte = text.split(" ")[0] if text else ""
                has_cs = "CS" in text
                votes, unanimous = _parse_report_votes(text)
                vote_str = " ".join(f"{v}{k}" for k, v in votes.items()) if votes else ""
                moved_out.append({
                    "billnumber": bn,
                    "title": title,
                    "chamber": achamber,
                ...
                if votes:
                    if unanimous:
                        report_unanimity["unanimous"] += 1
                    else:
                        report_unanimity["divided"] += 1
                    report_unanimity["reports"].append({
                        "billnumber": bn,
                        "committee": f"({achamber}) {cmte}",
                        "vote_str": vote_str,
                        "unanimous": unanimous,
                        "date": _format_status_date(jdate),
                    })
            ...
            # CS adopted
            if code in ("009", "122"):
                method = "UC" if code == "009" else "roll call"
                cmte = text.split(" ")[0] if text else ""
                cs_adopted.append({
                    "billnumber": bn,
                    "title": title,
    ...
    moved_out.sort(key=lambda x: x["date"], reverse=True)
    cs_adopted.sort(key=lambda x: x["date"], reverse=True)
    report_unanimity["reports"].sort(key=lambda x: x["date"], reverse=True)

    result = {
        "moved_out": moved_out,
        "cs_adopted": cs_adopted,
        "report_unanimity": report_unanimity,
    }
    cache.put("committee_activity", result)
    return result
