...
# NB: fetch_sponsor_statement() was removed deliberately. Sponsor
# statements are advocacy material written by the bill's sponsor;
# this dashboard exists to support neutral veto decisions, not
# re-platform the champion's framing. The PDF URL is also available
# directly from the BASIS Sponsors expansion (<SponsorStatement>),
# so if a future feature needs it, read it from there instead of
# scraping the akleg HTML.
